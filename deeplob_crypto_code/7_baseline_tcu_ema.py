#!/usr/bin/env python3
"""
DeepLOB-TCN Per-Symbol Training - EMA平滑目标变量版本
...
def train_single_symbol_worker(args):
    ...
    try:
        log_print(f"\n{'='*80}")
        log_print(f"🚀 开始训练: {symbol} (GPU {gpu_id}) - DeepLOB-TCN with EMA Smoothing")
...
def generate_final_report(df_results, output_dir, image_dir):
    ...
    plt.suptitle('DeepLOB-TCN Model with EMA Smoothing - Core Performance Metrics', 
...
def main():
    print("="*80)
    print("🚀 DeepLOB-TCN Per-Symbol Training with EMA Smoothing")
    print("="*80)
    ...
    # 路径 - 所有输出保存到7_models文件夹
    data_dir = Path('data_250801_250810')
    output_dir = Path('7_models')
    ...
    if n_gpus >= 4:
        ...
        print(f"   分配策略: GPU 0、1 -> 前一半标的")
        print(f"              GPU 2、3 -> 后一半标的")
    elif n_gpus >= 2:
    ...
    if results:
        ...
        # 保存CSV
        csv_file = output_dir / 'training_summary_ema.csv'
